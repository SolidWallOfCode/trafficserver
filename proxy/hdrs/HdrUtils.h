...
public:
   ...
   */
  HdrCsvIter(const char s = ',') : m_separator(s) {}
...
private:
  ...
  TextView m_csv;
  /// The terminating separator after @a csv, or the last character if no more separators.
  const char *m_csv_end{nullptr};
  /// The current sub-value index.
  int m_csv_index{0};
