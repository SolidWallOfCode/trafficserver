...
public:
  ...
  const ts::string_view scheme_get();
...
inline const ts::string_view
URL::scheme_get()
{
  ink_assert(valid());

  if (m_url_impl->m_scheme_wks_idx >= 0) {
    return ts::string_view{hdrtoken_index_to_wks(m_url_impl->m_scheme_wks_idx),
                            static_cast<size_t>(hdrtoken_index_to_length(m_url_impl->m_scheme_wks_idx))};
  } else {
    return ts::string_view{m_url_impl->m_ptr_scheme, m_url_impl->m_len_scheme};
  }
}
...
{
  ts::string_view ret = this->scheme_get();
  *length              = ret.size();
  return ret.data();
}
