...
RedisPublisher::setup_connection(const RedisEndpoint &re)
{
  ...
  for (int i = 0; i < static_cast<int>(m_redisConnectTries); ++i) {
    ...
    if (!my_context) {
      TSError("RedisPublisher::setup_connection. Connect to host: %s port: %d fail count: %d threadId: %lx",
      ...
      TSDebug(PLUGIN, "RedisPublisher::setup_connection. threadId: %lx Successfully connected to the redis instance.",
              my_id);
...
RedisPublisher::send_publish(RedisContextPtr &ctx, const RedisEndpoint &re, const Message &msg)
{
  ...
  for (int i = 0; i < static_cast<int>(m_redisPublishTries); ++i) {
    ...
    if (!current_reply) {
      TSError("RedisPublisher::send_publish. Unable to get a reply from the server for publish. threadId: %lx try: %d",
              my_id, (i + 1));
      ctx.reset(nullptr); // Clean up previous attempt

    } else if (REDIS_REPLY_ERROR == current_reply->type) {
      TSError("RedisPublisher::send_publish. Server responded with error for publish. threadId: %lx try: %d", my_id,
              i + 1);
