...
RedisSubscriber::setup_connection(int index)
{
  ...
  while (true) {
    ...
    if (!my_context) {
      TSError("RedisSubscriber::setup_connection. Connect to host: %s port: %d failed",
...
RedisSubscriber::run()
{
  ...
  while (true) {
    ...
    while (REDIS_OK == ::redisGetReply(my_context, reinterpret_cast<void **>(&current_reply))) {
      ...
      TSDebug(PLUGIN, "Redis request channel: %s message: %s", channel.c_str(), hex_str(data).c_str());
