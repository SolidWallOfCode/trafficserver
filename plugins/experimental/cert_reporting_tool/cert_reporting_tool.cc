...
CB_context_dump(TSCont, TSEvent, void *edata)
{
  ...
  static constexpr ts::TextView PLUGIN_PREFIX("cert_reporting_tool");

  ts::TextView tag(msg->tag, strlen(msg->tag));

  if (PLUGIN_PREFIX.isPrefixOf(tag)) {
    tag.split_prefix_at(PLUGIN_PREFIX.size());
    if (tag == "client") {
