  ...
  do {
    ...
    { // lock scoping
      ...
      if (alternate.is_frag_cached(fragment)) {
        ...
        // If the earliest fragment hasn't been cached yet (which also means no fragment has been cached) then we need
        // to adjust for that. This is done before the real earliest check so that if the content turns out to be
        // non-cacheable at all, we don't want to force an earliest when it was unnecessary.  Note: frag_offset can only
        // be off by ffs because it's based on an empty earliest or not.
        ...
        // If the earliest fragment isn't cached, we can't depend on fragment index from updateWriteStateFromRange
        // because it doesn't know if there is an empty earliest yet or not. The just previous check for the
        // fragment being cached is fine because if any fragment is cached, the earliest is also cached. It is
        // also the case that a fragment index of zero will alway be correct so we can skip this and go on with a
        // non-empty write of the earliest fragment. We wait to do this so that we don't write an empty earliest
        // unless there is no choice but to do so to cache other fragments.
