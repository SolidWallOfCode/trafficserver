...
TEST_CASE("bwstring std formats", "[libts][bwprint]")
{
  ...
  w.reset().print("{}", ts::bwf::As_Hex(v));
  REQUIRE(w.view() == "deadbeef");
  w.reset().print("{:x}", ts::bwf::As_Hex(v));
  REQUIRE(w.view() == "deadbeef");
  w.reset().print("{:X}", ts::bwf::As_Hex(v));
  REQUIRE(w.view() == "DEADBEEF");
  w.reset().print("{:#X}", ts::bwf::As_Hex(v));
  REQUIRE(w.view() == "0XDEADBEEF");
  w.reset().print("{} bytes {} digits {}", sizeof(double), std::numeric_limits<double>::digits10, ts::bwf::As_Hex(2.718281828));
  REQUIRE(w.view() == "8 bytes 15 digits 9b91048b0abf0540");

  INK_MD5 md5;
  w.reset().print("{}", ts::bwf::As_Hex(md5));
