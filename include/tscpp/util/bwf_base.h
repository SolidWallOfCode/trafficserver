...
namespace bwf
{
  ...
  inline auto
  Format::bind(ts::TextView fmt) -> TextViewExtractor
  {
    return {fmt};
  }
  ...
  inline void
  BoundNames::capture(BufferWriter &, ts::bwf::Spec const &, std::any const &) const
  {
  }
  ...
  template <typename F>
  auto
  arg_capture(F &&f, BufferWriter &, Spec const &, std::any &&, ts::meta::CaseTag<0>) -> void
  {
    throw std::runtime_error("Capture specification used in format extractor that does not support capture");
  }
  template <typename F>
  auto
  arg_capture(F &&f, BufferWriter &w, Spec const &spec, std::any &&value, ts::meta::CaseTag<1>)
      ...
      if (0 <= spec._idx) {
        if (spec._idx < N) {
          if (spec._type == bwf::Spec::CAPTURE_TYPE) {
            bwf::arg_capture(f, lw, spec, bwf::Tuple_Nth(args, static_cast<size_t>(spec._idx)), ts::meta::CaseArg);
          } else {
