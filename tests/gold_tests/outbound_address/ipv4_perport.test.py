...
import os
Test.Summary = '''
Verify port specific outbound addresses are used in round robin order when
...
# Define default ATS
# If select_ports == False, microserver uses port 8080
...
ts2.Disk.remap_config.AddLine(
    'map example.com/ http://127.0.0.1:{0}'.format(server.Variables.Port)
)
...
ts.Disk.records_config.update({
    ...
    'proxy.config.http.server_ports': '8080:ip-out=127.0.0.15;127.0.0.16;127.0.0.17',
...
# ATS1 should use the port specific outbound addresses
...
tr.Processes.Default.ReturnCode = 0
tr.Processes.Default.Streams.stdout = "gold/client-ips-8080.gold"
