...
testsForReadReqHdrHook(TSHttpTxn txn)
{
  ...
  if (!urlStr) {
    ...
    logFile << ts::string_view(urlStr, urlLength) << std::endl;
  ...
  logFile << "TSHttpHdrEffectiveUrlBufGet():  ";
  {
    ...
    if (TSHttpTxnClientReqGet(txn, &hbuf, &hloc) != TS_SUCCESS) {
      ...
      auto ps_func = [=]() -> void { TSHandleMLocRelease(hbuf, TS_NULL_MLOC, hloc); };
      ts::PostScript<decltype(ps_func)> ps(ps_func);
      ...
      if (TSHttpHdrEffectiveUrlBufGet(hbuf, hloc, nullptr, 0, &url_length) != TS_SUCCESS) {
        ...
        std::vector<char> s(url_length + 4);

        std::memcpy(s.data() + url_length, "yada", 4);
        ...
        } else if (std::memcmp(s.data() + url_length, "yada", 4) != 0) {
          logFile << "overwrite" << std::endl;

        } else {
          logFile << ts::string_view(s.data(), url_length) << std::endl;
        }
      }
    }
  }
...
{
  logFile << "Transaction: event=" << int(event) << std::endl;

  TSDebug(PIName, "Transaction: event=%d eventData=%p", event, eventData);
...
{
  logFile << "Global: event=" << int(event) << std::endl;

  TSDebug(PIName, "Global: event=%d eventData=%p", event, eventData);
...
// In 7.1, if TSContDestroy() is called during static destruction at shutdown, it causes a Seg Fault.  This problem does
// not occur in 9.0
//
#if 0
