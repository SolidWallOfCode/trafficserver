...
of which is an association between an enumeration, a primary name, and secondary names, the last of
which is optional. All names and enumerations must be unique across the :class:`Lexicon`. The
array operator is used to do the conversions. When indexed by an enumeration, the primary name for that enumeration is returned. When index by a
