...
*/
size_t
unescape_string(char *text, size_t len)
{
  size_t zret = len;
...
namespace config
{
  ...
  void
  Builder::groupClose(Token const &)
  {
    _v = _v.getParent();
  }
  void
  Builder::groupName(Token const &token)
  {
    _name.set(token._s, token._n);
  }
  void
  Builder::listOpen(Token const &token)
  {
    _v = _v.makeList(_name);
    _v.setSource(token._loc._line, token._loc._col);
  }
  void
  Builder::listClose(Token const &)
  {
    _v = _v.getParent();
  }

  void
  Builder::pathOpen(Token const &)
  {
    _path.reset();
    _extent.reset();
  }
  void
  Builder::pathTag(Token const &token)
  {
  ...
  void
  Builder::pathIndex(Token const &token)
  {
  ...
  void
  Builder::literalValue(Token const &token)
  {
    Rv<Value> cv;
  ...
  void
  Builder::invalidToken(Token const &)
  {
  }
}
} // namespace ts::config
