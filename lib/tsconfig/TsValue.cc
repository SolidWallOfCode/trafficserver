...
namespace config
{
  ...
  detail::ValueItem &detail::ValueTable::operator[](ValueIndex idx)
  {
  ...
  Value Value::operator[](size_t idx) const
  {
    ...
    if (item && idx < item->_children.size()) {
      ...
      if (PathValue == zret.getType()) {
        zret = _config.getRoot().find(_config._table[zret._vidx]._path);
      }
    }
    return zret;
  }

  Value Value::operator[](ConstBuffer const &name) const
  {
    Value zret;
    detail::ValueItem const *item = this->item();
    if (item) {
  ...
  Value::find(ConstBuffer const &path)
  {
    ...
    for (x = parser.parse(&elt); zret && Path::Parser::EOP != x && Path::Parser::ERROR != x; x = parser.parse(&elt)) {
      if (Path::Parser::TAG == x) {
        zret = zret[elt];
      } else if (Path::Parser::INDEX == x) {
        zret = zret[elt._size];
      } else {
        zret.reset();
      }
    }
    if (Path::Parser::EOP != x) {
      zret.reset();
    }
    return zret;
  }
