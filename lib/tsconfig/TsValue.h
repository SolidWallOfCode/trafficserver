...
namespace config
{
  ...
  public:
    ...
    self &append(ConstBuffer const &tag ///< Text of tag.
                 );
    /// Append a numeric index to the path.
    self &append(size_t idx ///< Index.
                 );
    ...
    ConstBuffer const &operator[](size_t index ///< Element index.
                                  ) const;
    ...
    */
    ...
    public:
      ...
      /** Construct with input.
          ...
          @see setInput.
      */
      Parser(ConstBuffer const &text ///< Input text.
             );
      ...
      */
      self &setInput(ConstBuffer const &text ///< Input buffer.
                     );
  ...
  namespace detail
  {
    ...
    */
    ...
    public:
      ...
      ValueItem const &operator[](ValueIndex idx ///< Index of item.
                                  ) const;
      /// Access by index.
      /// @return The value item at index @a idx.
      ValueItem &operator[](ValueIndex idx ///< Index of item.
                            );
      ...
      */
      Rv<ValueIndex> make(ValueIndex parent,                    ///< Index of parent for item.
  ...
  */
  ...
  public:
    ...
    */
    Value operator[](size_t idx ///< Index of child value.
                     ) const;
    ...
    */
    ...
    /** Root value child access by @a name.
        @return The child or a @c Void value if there is no child with @a name.
    */
    Value operator[](char const *name ///< Null terminated string.
                     ) const;

    /** Find a value.
        @return The value if found, an void valid if not.
    */
    Value find(char const *path ///< configuration path to value.
               );
    ...
    */
    static Rv<self> loadFromPath(char const *path ///< file system path.
                                 );
    /** Allocate a local buffer of size @a n.
        ...
        @return The allocated buffer.
    */
    Buffer alloc(size_t n ///< requested size of buffer.
                 );
  ...
  */
  ...
  public:
    ...
    */
    ...
    self &setText(ConstBuffer const &text);
    ...
    */
    ...
    Rv<Value> makeString(ConstBuffer const &text,                      ///< String content.
                         ConstBuffer const &name = detail::NULL_BUFFER ///< Optional name of value.
                         );
    /// Create an @c Integer value.
    Rv<Value> makeInteger(ConstBuffer const &text,                      ///< Text of number.
                          ConstBuffer const &name = detail::NULL_BUFFER ///< Optional name of value.
                          );
    /// Create a @c Group value.
    Rv<Value> makeGroup(ConstBuffer const &name = detail::NULL_BUFFER ///< Optional name of value.
                        );
    /// Create a @c List value.
    Rv<Value> makeList(ConstBuffer const &name = detail::NULL_BUFFER ///< Optional name of value.
                       );
    /// Create a @c Path value.
    Rv<Value> makePath(Path const &path,                             ///< Path.
                       ConstBuffer const &name = detail::NULL_BUFFER ///< Optional name of value.
                       );
    /// Create a child by type.
    /// Client must fill in any other required elements.
    Rv<Value> makeChild(ValueType type,                               ///< Type of child.
                        ConstBuffer const &name = detail::NULL_BUFFER ///< Optional name of value.
                        );
    //@}
    ...
    /** Find a value.
        @return The value if found, an void valid if not.
    */
    Value find(ConstBuffer const &path ///< Path relative to this value.
               );
    /** Find a value.
        @return The value if found, an void valid if not.
    */
    Value find(char const *path ///< Path relative to this value.
               );
    /** Find a value using a precondensed path.
        @return The value if found, an void valid if not.
    */
    Value find(Path const &path ///< Path relative to this value.
               );
    ...
    */
    ...
    /// Set source line.
    /// @return @c this object.
    self &setSourceLine(int line ///< Line in source stream.
                        );
    /// Set source column.
    /// @return @c this object.
    self &setSourceColumn(int col ///< Column in source stream.
                          );
    /// Set the source location.
    self &setSource(int line, ///< Line in source stream.
  ...
  protected:
    ...
    Value(Configuration cfg,      ///< Source configuration.
  ...
  inline bool
  Value::hasValue() const
  {
    return _config && _vidx != detail::NULL_VALUE_INDEX;
  }
  inline Value::operator detail::PseudoBool::Type() const
  {
    return this->hasValue() ? detail::PseudoBool::TRUE : detail::PseudoBool::FALSE;
  }
  inline bool Value::operator!() const { return !this->hasValue(); }
  inline ValueType
  Value::getType() const
  {
    return this->hasValue() ? _config._table[_vidx]._type : VoidValue;
  }
  inline ConstBuffer const &
  Value::getText() const
  {
    return this->hasValue() ? _config._table[_vidx]._text : detail::NULL_CONST_BUFFER;
  }
  inline Value &
  Value::setText(ConstBuffer const &text)
  {
  ...
  inline bool
  Path::Parser::hasInput() const
  {
    return _input._ptr && _input._ptr + _input._size > _c;
  }
  ...
  inline Value
  Configuration::find(char const *path)
  {
    return this->getRoot().find(path);
  }
  inline Buffer
  Configuration::alloc(size_t n)
  {
    return _table.alloc(n);
  }
  inline size_t
  Configuration::childCount() const
  {
