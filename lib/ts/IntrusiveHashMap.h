...
public:
  ...
  */
  iterator erase(const_iterator const &loc);

  /// Remove all elements in the @c range @a r.
  iterator erase(const_range const &r);

  /// Remove all elements in the range (start, limit]
  iterator erase(const_iterator const &start, const_iterator const &limit);
...
template <typename H>
auto
IntrusiveHashMap<H>::erase(const_iterator const &loc) -> iterator
{
  value_type *v     = loc;
...
template <typename H>
bool
IntrusiveHashMap<H>::erase(const value_type *value)
{
...
template <typename H>
auto
IntrusiveHashMap<H>::erase(const_iterator const &start, const_iterator const &limit) -> iterator
{
  auto spot{start};
...
template <typename H>
auto
IntrusiveHashMap<H>::erase(const_range const &r) -> iterator
{
