...
public:
  ...
  iterator erase(const const_iterator &loc);

  /// Take elements out of the list.
  /// Remove elements start with @a start up to but not including @a limit.
  /// @return @a limit
  iterator erase(const const_iterator &start, const const_iterator &limit);
...
template <typename L>
auto
IntrusiveDList<L>::erase(const const_iterator &loc) -> iterator
{
...
template <typename L>
auto
IntrusiveDList<L>::erase(const const_iterator &first, const const_iterator &limit) -> iterator
{
