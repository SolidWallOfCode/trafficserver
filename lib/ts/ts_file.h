...
namespace file
{
  ...
  public:
    ...
    path(ts::string_view src);
    //  template < typename ... Args > explicit path(ts::string_view base, Args... rest);
    ...
    self_type &operator=(ts::string_view p);
    ...
    self_type &operator/=(ts::string_view that);
  ...
  inline path::path(ts::string_view base) : _path(base.data(), base.size()) {}

  inline path::path(std::string &&that) : _path(std::move(that)) {}

  inline path &
  path::operator=(ts::string_view p)
  {
    _path.assign(p.data(), p.size());
    return *this;
  }
  ...
  {
    return *this /= ts::string_view(that._path);
  }
  ...
  */
  ...
  inline path
  operator/(const path &lhs, ts::string_view rhs)
  {
    return path(lhs) /= rhs;
  }

  inline path
  operator/(path &&lhs, ts::string_view rhs)
  {
    return path(std::move(lhs)) /= rhs;
  }
