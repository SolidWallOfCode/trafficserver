...
#include <cassert>
#include <ts/ts.h>
#include "tscpp/TransactionPlugin.h"
#include "utils_internal.h"
#include "tscpp/noncopyable.h"
#include "logging_internal.h"

using namespace tscpp;
using tscpp::TransactionPlugin;

/**
 * @private
 */
struct tscpp::TransactionPluginState : noncopyable {
...
handleTransactionPluginEvents(TSCont cont, TSEvent event, void *edata)
{
  ...
  tscpp::utils::internal::invokePluginForEvent(plugin, txn, event);
  return 0;
}
...
TransactionPlugin::registerHook(Plugin::HookType hook_type)
{
  ...
  TSHttpHookID hook_id = tscpp::utils::internal::convertInternalHookToTsHook(hook_type);
