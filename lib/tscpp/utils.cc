...
std::string
tscpp::utils::getIpString(const sockaddr *sockaddress)
{
...
uint16_t
tscpp::utils::getPort(const sockaddr *sockaddress)
{
  if (sockaddress == nullptr) {
    LOG_ERROR("Cannot work on nullptr sockaddress");
    return 0;
...
std::string
tscpp::utils::getIpPortString(const sockaddr *sockaddress)
{
  if (sockaddress == nullptr) {
    LOG_ERROR("Cannot work on nullptr sockaddress");
    return std::string();
